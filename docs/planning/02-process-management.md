...
- ~~"프로세스"를 제품 단위(ProcessRouting)로만 정의할지, 제품과 무관한 공통 공정 템플릿도 별도 관리 대상으로 둘지~~ → **답변**: 원자재가 공정 도중 갈라져 서로 합쳐지지 않는 여러 완제품이 되는 경우(예: 같은 상류 설비를 공유하는 co-product 분기), `ProcessRouting`을 "제품 1개 전용"으로 유지하면 공유 상류 공정을 제품마다 중복 정의해야 하고, 그러면 여러 제품이 같은 설비/적재공간을 두고 실제로 경쟁하는 상황이 시뮬레이션에 반영되지 않는다(각 제품이 독립된 SimPy 컴파일 결과로 돌기 때문). 해결 방향은 `RoutingProductLink`(하나의 `ProcessRouting` = 공유 네트워크, 제품마다 entry/terminal step_no만 다르게 매핑)이며, 구체적 스키마/컴파일러/시뮬레이터/API 변경안은 [`07-schema-gap-review.md`](./07-schema-gap-review.md) 6장 참고 (설계 완료, 구현 대기 — 3-1 co-product 비율 분기와 함께 구현해야 완성됨).
